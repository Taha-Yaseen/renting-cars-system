...
export default function ExtendRentalForm({ rental, car, client, onSubmit, onCancel }) {
        ...
        <div className="rounded-lg bg-indigo-50 px-4 py-3">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-indigo-700">
              ...
              {!isOpenEnded && (
                <p className="mt-1">{t('rentals.additionalCharge', { amount: formatNumber(additionalCost, locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) })}</p>
              )}
              <p className="mt-1 text-base font-bold">
                {t('rentals.newTotal', { total: formatNumber(newTotal, locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) })}
              </p>
            </div>
          </div>
        </div>
