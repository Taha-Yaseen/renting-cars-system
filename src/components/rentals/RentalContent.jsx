...
const RENTAL_FILTERS = ['all', 'Active', 'Overdue', 'Completed']

export default function RentalContent({ openAddOnMount = false }) {
  ...
  const filtered = rentals.filter((r) => {
    ...
    if (search.trim()) {
      ...
      const carText = car
        ? `${car.make} ${car.model} ${car.licensePlate ?? ''}`.toLowerCase()
        : ''
      const clientText = client
        ? `${client.fullName} ${client.phone ?? ''}`.toLowerCase()
        : ''
  ...
  const sorted = [...filtered].sort((a, b) => {
    const statusOrder = { Active: 0, Overdue: 1, Completed: 2 }
    const statusDiff =
  ...
  const handleExtend = async (newEndDate) => {
  ...
  const receiptLabels = {
    ...
    formatDays: (count) => t('rentals.receipt.days', { count: formatNumber(count, locale) }),
  ...
  const handleReturnCar = async (rental) => {
      ...
      <PageHeader
        ...
        description={
          hasExtraFilters || filter !== 'all'
            ? t('rentals.countFiltered', { shown: formatNumber(sorted.length, locale), total: formatNumber(rentals.length, locale) })
