...
const CLIENT_STATUSES: ClientStatus[] = ['Active', 'Suspended']

interface Props {
  client?: Client | null
  onSubmit: (data: Partial<Omit<Client, 'id'>>) => void
  onCancel: () => void
}
...
export default function ClientForm({ client, onSubmit, onCancel }: Props) {
  ...
  const field = (name: keyof Pick<ClientFormState, 'fullName' | 'phone'>, label: string, type = 'text') => (
    <div>
